...
void effinit(enum effrefresh effref,int d){
	int i;
	...
	if(effref&EFFREF_ALL) for(i=0;i<nimg;i++) effinitimg(d,i);
	else if(effref&EFFREF_IMG) effinitimg(d,dpl.pos.imgi);
...
void dplmove(int d){
	...
	effinit(EFFREF_ALL|EFFREF_FIT,d);
}
